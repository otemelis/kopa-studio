...
2. **Foundation — in progress.** The independent `apps/tools` Next.js application has App Router, TypeScript, Tailwind, cookie-based Supabase Auth, server authorization, repositories, services, validation, tests, and a protected shell.
3. **Read-only parity.** Compare app list/detail, keywords, rankings, competitors, localizations, insights, and App Store Connect data with the legacy console before enabling writes.
